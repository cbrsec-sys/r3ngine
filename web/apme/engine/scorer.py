...
class Scorer:
    ...
    def score(self, path: AttackPath, path_metadata: Dict[str, Any]) -> float:
        ...
        # 3. Access & Privilege Metrics
        privilege = path_metadata.get("privilege_gained", "none")
        privilege_score = PRIVILEGE_MAP.get(privilege, 0.0)

        # 4. Impact Metrics (Blast Radius & Sensitivity)
        blast_radius = min(path_metadata.get("blast_radius", 0) / 50.0, 1.0)
        ...
        impact_score = (blast_radius * 0.4) + (sensitivity_score * 0.6)
        ...
        logger.debug(
            ...
            f"(sev={severity_score:.2f} expl={exploitability:.2f} "
            f"len={length_score:.2f} impact={impact_score:.2f})"
        )
        return score
