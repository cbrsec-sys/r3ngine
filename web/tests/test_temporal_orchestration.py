...
class TestTemporalOrchestration(TestCase):
    ...
    def test_recover_stuck_scans_restarts_running_and_failed(self, mock_get_client, mock_resume_scan):
        ...
        # automatic=True marks these as budget-consuming auto-recoveries, unlike
        # a manual resume from the UI.
        mock_resume_scan.assert_any_call(scan_running_stuck.id, automatic=True)
        mock_resume_scan.assert_any_call(scan_failed.id, automatic=True)
