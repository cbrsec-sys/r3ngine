...
async def check_scan_queue_status_activity(scan_id: int, queue_type: str) -> bool:
    ...
    """
    # database_sync_to_async, not asgiref's sync_to_async: this is the only async
    # activity, so its ORM calls run in an asgiref thread that
    # DjangoAwareThreadPoolExecutor never touches. Nothing refreshed that
    # thread's cached connection, and once Postgres closed it (an idle timeout,
    # a restart) every later call raised "connection already closed" — the first
    # step of MasterScanWorkflow failed and scans sat at 0% while reading RUNNING.
