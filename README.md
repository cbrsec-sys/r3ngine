...
<p align="center">
  <a href="https://github.com/whiterabb17/r3ngine-mcp" target="_blank">
    <img src="https://img.shields.io/badge/r3ngine--mcp-v1.1.0-blue.svg?logo=none" alt="r3ngine MCP v1.1.0" />
  </a>
  &nbsp;
...
The [r3ngine-mcp](https://github.com/whiterabb17/r3ngine-mcp) companion is the official Model Context Protocol server for this instance. Cursor, Claude Desktop, VS Code, or any MCP-capable agent can list recon data, run a **singular pipeline tool** on one asset (with host-local `tool_args` schemas), propose follow-up batches, and verify OSINT staging — with a hashed API key and no delete/edit tools, database URLs, or filesystem mounts. Identity, keys, sessions, and the request/response audit chain live in **Settings → MCP Access**. The sidecar speaks MCP only and is an HTTP client of `/api/mcp/`.
...
🔌&nbsp;&nbsp; **MCP Access**: official [r3ngine-mcp](https://github.com/whiterabb17/r3ngine-mcp) **v1.1.0** server so IDE agents can read recon, run singular tools with validated `tool_args`, propose follow-ups, and verify OSINT staging through `/api/mcp/` — hashed keys, sessions, and an audit chain in Settings. The UI mirrors singular runs from the Subdomains tab (**Run single tool**).
...
*   **Multi-Source Breach Intelligence**: **WhatBreach** (DeHashed, Pastebin, Hunter.io discovery — all modules except HIBP) runs alongside the existing HIBP scraper; each `EmailBreach` row carries a `source` badge distinguishing the two tools. Optional database download is opt-in. **CredSpy** runs post-crawl when Microsoft Exchange/Entra infrastructure is detected (MX records, autodiscover subdomains), enumerating account existence and auth capabilities via Azure GetCredentialType — proxy-mandatory to protect the operator's IP. All four report templates present breach findings grouped per email address with per-source badges and an inline CredSpy sub-table.
...
2. Install the companion server from this checkout ([repo](https://github.com/whiterabb17/r3ngine-mcp), [v1.1.0](https://github.com/whiterabb17/r3ngine-mcp/releases)):
